...
y1=sin(phi)*sint';
z1=ones(np,1)*cost';
x=zeros(np,nt,nr+nr_ic);
y=zeros(np,nt,nr+nr_ic);
z=zeros(np,nt,nr+nr_ic);
for i=1:nr+nr_ic
x(:,:,i)=r(i)*x1(:,:);
y(:,:,i)=r(i)*y1(:,:);
z(:,:,i)=r(i)*z1(:,:);
...
innercore=surf(r(nr)*x1(:,:),r(nr)*y1(:,:),r(nr)*z1(:,:),Br(:,:,nr)/3)
set(innercore,'specularstrength',0.2);
hold on


% render the CMB radial field
cmbr=surf(r(1)*x1(:,1:nt/2),r(1)*y1(:,1:nt/2),r(1)*z1(:,1:nt/2),Br(:,1:nt/2,1))
caxis([-6,6])
alphad=(Br(:,1:nt/2,1)).^2;
...
li=plot3(r(nr)*sint*cos(longitude),r(nr)*sint*sin(longitude),r(nr)*cost)
set(li,'color','k')
     set(li,'linewidth',2)
end
...
li=plot3(r(nr)*sin(latitude)*cos(phi),r(nr)*sin(latitude)*sin(phi),r(nr)*cos(latitude)*ones(np,1))
set(li,'color','k')
set(li,'linewidth',2)
end
...
earthsurf=surf(r(nr)*x1(:,:),r(nr)*y1(:,:),r(nr)*z1(:,:),150*sc2)
caxis([-6 6])
...
% render the CMB radial field
cmbr=surf(r(1)*x1(np/2:np,:),r(1)*y1(np/2:np,:),r(1)*z1(np/2:np,:),Br(np/2:np,:,1))
caxis([-6,6])
alphad=(Br(np/2:np,:,1)).^2;
set(cmbr,'FaceAlpha','flat');
set(cmbr,'AlphaData',alphad);
set(cmbr,'specularstrength',0.2);
shading('flat')
